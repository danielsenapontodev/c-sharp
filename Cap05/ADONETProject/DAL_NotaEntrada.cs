...
namespace ADONETProject
{
    public class DAL_NotaEntrada
    {
        ...
        {
            var command = new SqlCommand("insert into NOTASDEENTRADA(IdFornecedor, Numero, DataEmissao, DataEntrada)" +
        ...
        private void InsertProdutosNotaDeEntrada(long? idNotaEntrada, IList<ProdutoNotaEntrada> produtos)
        {
            var command = new SqlCommand("insert into PRODUTOSNOTASDEENTRADA(IdNotaDeEntrada, IdProduto," +
                " PrecoCustoCompra, QuantidadeCompra) values(@IdNotaDeEntrada, @IdProduto, @PrecoCustoCompra, @QuantidadeCompra)", connection);
            connection.Open();
        ...
        internal object GetAllAsDataTable()
        {
        ...
        public NotaEntrada GetById(long id)
        {
            ...
            var command = new SqlCommand("select id, idfornecedor, numero, dataemissao, dataentrada from NOTASDEENTRADA" +
        ...
        private void InsertProduto(NotaEntrada notaEntrada, ProdutoNotaEntrada produto)
        {
            ...
            var command = new SqlCommand("insert into PRODUTOSNOTASDEENTRADA(idnotadeentrada, idproduto, precocustocompra," +
                "quantidadecompra) values(@idnotadeentrada, @idproduto, @precocustocompra, @quantidadecompra)", this.connection);
